...
public class FluidCanisterScreen extends AbstractContainerScreen<FluidCanisterMenu> {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (isMouseOverButton(mouseX, mouseY, x + EXTRACT_BUTTON_X, y + EXTRACT_BUTTON_Y, EXTRACT_BUTTON_SIZE, EXTRACT_BUTTON_SIZE)) {
            PacketDistributor.sendToServer(new ExtractionPacket(ExtractionPacket.ExtractionType.CANISTER), new CustomPacketPayload[0]);
            return true;
        }
    ...
    protected void renderTooltip(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        int y = (this.height - this.imageHeight) / 2;
        if (isMouseOverButton(mouseX, mouseY, x + EXTRACT_BUTTON_X, y + EXTRACT_BUTTON_Y, EXTRACT_BUTTON_SIZE, EXTRACT_BUTTON_SIZE)) {
            guiGraphics.renderTooltip(this.font, Component.translatable("gui.realfilingreborn.extract_fluid"), mouseX, mouseY);
        }
    }
    ...
    protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        guiGraphics.drawString(this.font, instruction, scaledX, 25, 2763485, false);
