...
public class FluidCabinetMenu extends AbstractContainerMenu {

    public final FluidCabinetBlockEntity blockEntity;
    ...
    public FluidCabinetMenu(int containerId, Inventory inv, FriendlyByteBuf extraData) {
        this(containerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()));
    }

    public FluidCabinetMenu(int containerId, Inventory inv, BlockEntity blockEntity) {
        super(RFRMenuTypes.FLUID_CABINET_MENU.get(), containerId);
        this.blockEntity = (FluidCabinetBlockEntity) blockEntity;
        this.level = inv.player.level();
        addPlayerInventory(inv);
        addPlayerHotbar(inv);

        for (int i = 0; i < TE_INVENTORY_SLOT_COUNT; i++) {
            addSlot(new FluidCabinetSlot(this.blockEntity, i, 53 + i * 18, 23));
        }
    }
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if (index < VANILLA_SLOT_COUNT) {
            if (!(sourceStack.getItem() instanceof FluidCanisterItem)) return ItemStack.EMPTY;
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), player, RFRBlocks.FLUID_CABINET.get());
    }
    ...
    private static class FluidCabinetSlot extends Slot {
        private final FluidCabinetBlockEntity be;
        private final int index;

        public FluidCabinetSlot(FluidCabinetBlockEntity be, int index, int x, int y) {
